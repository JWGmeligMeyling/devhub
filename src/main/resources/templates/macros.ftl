			...
			[#assign type=entries[entry]]
				...
				[#if type = "FOLDER"]
					[#if path?? && path?has_content]
							<i class="folder glyphicon glyphicon-folder-open"></i> <a href="/projects/${group.repositoryName}/commits/${commit.commit}/tree/${path}/${entry}">${entry}</a>
					[#else]
							<i class="folder glyphicon glyphicon-folder-open"></i> <a href="/projects/${group.repositoryName}/commits/${commit.commit}/tree/${entry}">${entry}</a>
					[/#if]
				[#elseif type = "TEXT"]
					[#if path?? && path?has_content]
							<i class="text glyphicon glyphicon-file"></i> <a href="/projects/${group.repositoryName}/commits/${commit.commit}/blob/${path}/${entry}">${entry}</a>
					[#else]
							<i class="text glyphicon glyphicon-file"></i> <a href="/projects/${group.repositoryName}/commits/${commit.commit}/blob/${entry}">${entry}</a>
					[/#if]
				[#else]
					[#if path?? && path?has_content]
							<i class="binary glyphicon glyphicon-save"></i> <a href="/projects/${group.repositoryName}/commits/${commit.commit}/blob/${path}/${entry}">${entry}</a>
					[#else]
							<i class="binary glyphicon glyphicon-save"></i> <a href="/projects/${group.repositoryName}/commits/${commit.commit}/blob/${entry}">${entry}</a>
					[/#if]
				[/#if]
							</td>
						</tr>
...
[#macro renderTreeBreadcrumb group commit repository path]
	...
	<a href="/projects/${group.repositoryName}/commits/${commit.commit}/tree">group-${group.groupNumber}</a> /
	[#list pathParts as pathPart]
		[#if pathPart_has_next]
					<a href="/projects/${group.repositoryName}/commits/${commit.commit}/tree/[#list 0..pathPart_index as i]${pathParts[i]}[#if i_has_next]/[/#if][/#list]">${pathPart}</a> /
...
[#macro renderCommitHeader i18n group commit currentView]
			<ol class="breadcrumb">
				...
				<li><a href="/projects/${group.repositoryName}">${group.getGroupName()}</a></li>
				...
				<span class="view-picker">
					<div class="btn-group">
						...
						<ul class="dropdown-menu" role="menu">
							<li><a href="/projects/${group.repositoryName}/commits/${commit.commit}/diff">View diff</a></li>
							<li><a href="/projects/${group.repositoryName}/commits/${commit.commit}/tree">View files</a></li>
	[#if states.hasFinished(commit.getCommit())]
